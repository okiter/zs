    ...
    <div id="menu-list">
        <ul id="menu-ul">
            <?php if(is_array($menus)): $i = 0; $__LIST__ = $menus;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$menu): $mod = ($i % 2 );++$i; if(($menu["level"]) == "1"): ?><li class="explode" key="02_cat_and_goods" name="menu">
                <?php echo ($menu["name"]); ?>
                    <ul>
                        <?php if(is_array($menus)): $i = 0; $__LIST__ = $menus;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$menu_1): $mod = ($i % 2 );++$i;?><!--由于查找的为 子菜单. 所以-->
                            <?php if(($menu_1["parent_id"]) == $menu['id']): ?><li class="menu-item"><a href="<?php echo U($menu_1['url']);?>" target="main-frame"><?php echo ($menu_1["name"]); ?></a></li><?php endif; endforeach; endif; else: echo "" ;endif; ?>
                    </ul>
